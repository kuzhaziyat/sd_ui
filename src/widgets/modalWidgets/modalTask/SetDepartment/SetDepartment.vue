<script lang="ts" setup>
  ...
  function doAction() {
    ...
    doRequest("/tasks/update/department", {
  ...
  <div class="task__actions" @click="close" @keyup.esc="$emit('close')" ref="__wrp" tabindex="0">
    <div class="setdepartment">
      <div class="setdepartment__wrp">
        <h1>Выберите структурное подразделение:</h1>
        <select v-model="selected">
          <option v-for="(dep, index) in departments" :key="index" :value="dep.id">{{ dep.name }}</option>
        </select>
        <button :class="{ 'active-button': selected }" :disabled="!selected" @click="doAction">Назначить</button>
      </div>
